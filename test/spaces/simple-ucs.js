...
export const sLab = {
  id: "slab",
  toXYZ(Lab, out = vec3()) {
    const invPower = 1.0 / 0.43;
    const sLMSp = transform(Lab, sLab_to_sLMSp_M);
    const sLMS = sLMSp.map((v) => {
      return v >= 0 ? Math.pow(v, invPower) : -Math.pow(-v, invPower);
    });
    return transform(sLMS, sLMS_to_XYZ_D65_M, out);
  },
  fromXYZ(xyz, out = vec3()) {
    const power = 0.43;
    const sLMS = transform(xyz, XYZ_D65_to_sLMS_M);
    const sLMSp = sLMS.map((v) => {
      return v >= 0 ? Math.pow(v, power) : -Math.pow(-v, power);
    });
    return transform(sLMSp, sLMSp_to_sLab_M, out);
  },
};
